...
function pollProcessing() {
  const interval = setInterval(async () => {
    ...
    if (status.state === 'processing') {
      ...
      procCount.textContent = `${status.done} / ${status.total}`;
    }
...
function renderTriviaQuestion() {
  ...
  const qNum = parentQ.id !== undefined ? ('Q' + (parentQ.id + 1)) : ('Q' + (currentIndex + 1));
  document.getElementById('quiz-qnum').textContent = 'Exam Q' + (parentQ.id + 1) + ' · trivia ' + (currentIndex + 1) + '/' + quizState.total;
  ...
  // Auto-navigate NBME tab — use stored absolute question number
  const absNavNum = parentQ.absoluteId || (parentQ.id + 1);
  if (SETTINGS && SETTINGS.autonav && isFirstForParent && absNavNum !== window._lastNavNum) {
    window._lastNavNum = absNavNum;
    chrome.runtime.sendMessage({ type: 'NAVIGATE_TO_QUESTION', questionNum: absNavNum });
  }
...
// Collect button handler - reads from storage what content script auto-scraped
...
/* ─── QUIZ SETTINGS ─── */
let SETTINGS = { autonav: true, timer: true, timelimit: 45, readAloud: false, readExplain: false };
...
// Patch renderTriviaQuestion to read question after render
// Use MutationObserver on quiz-question-text since we can't reassign function declarations
const _readObserver = new MutationObserver(() => {
  const el = document.getElementById('quiz-question-text');
  if (el && el.textContent && el.textContent.trim().length > 10) {
    readQuestionAloud(el.textContent.trim());
  }
});
const _qText = document.getElementById('quiz-question-text');
if (_qText) _readObserver.observe(_qText, { childList: true, characterData: true, subtree: true });
