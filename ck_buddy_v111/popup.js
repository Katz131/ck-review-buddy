...
function _explSpeak() {
  ...
  if (window._cachedVoices && window._cachedVoices.length) u.voice = window._cachedVoices[0];
  u.rate = 0.95;
  window.speechSynthesis.speak(u);
}
document.getElementById('btnReadExplanation').addEventListener('click', function() {
  var item = quizState.triviaQueue && quizState.triviaQueue[quizState.currentIndex - 1];
  _explTTSText = (item && item.parentQ && item.parentQ.explanation) ? item.parentQ.explanation : '';
  if (!_explTTSText) { document.getElementById('btnReadExplanation').textContent = '🔊 No explanation found'; return; }
  _explSpeak();
});
