...
(function() {
  ...
  function scrapeCurrentQuestion(index) {
    ...
    // 5. Marked/flagged state (AMBOSS uses a bookmark/flag icon on each question)
    var isMarked = false;
    try {
      var markSel = [
        '[data-e2e-test-id*="bookmark" i][data-e2e-test-id*="active" i]',
        '[data-e2e-test-id*="mark" i][aria-pressed="true"]',
        'button[aria-label*="Unmark" i]',
        'button[aria-label*="Remove bookmark" i]',
        '[class*="bookmark"][class*="active"]',
        '[class*="marked"]:not([class*="question"])'
      ].join(',');
      var mk = document.querySelector(markSel);
      if (mk) isMarked = true;
      if (!isMarked) {
        // Fallback: any button whose visible/aria text says "Unmark"
        var btns = document.querySelectorAll('button, a, [role="button"]');
        for (var bi = 0; bi < btns.length; bi++) {
          var b = btns[bi];
          var lbl = (b.getAttribute('aria-label') || b.textContent || '').toLowerCase();
          if (lbl.indexOf('unmark') >= 0 || lbl.indexOf('remove bookmark') >= 0) { isMarked = true; break; }
        }
      }
    } catch(e) {}
