...
document.getElementById('btnReadExplanation').addEventListener('click', function() {
  ...
  var trivia = item && item.trivia;
  // Fallback chain: parent raw explanation → trivia explanation → analysis likelyMisconception → questionText + correct answer
  var txt = '';
  if (parentQ && parentQ.explanation && parentQ.explanation.trim().length > 5) {
    txt = parentQ.explanation;
  } else if (trivia && trivia.explanation && trivia.explanation.trim().length > 5) {
    txt = trivia.explanation;
  } else if (parentQ && parentQ.analysis && parentQ.analysis.likelyMisconception) {
    txt = parentQ.analysis.likelyMisconception;
  } else if (parentQ && parentQ.questionText) {
    txt = parentQ.questionText + ' Correct answer: ' + (parentQ.correctAnswer || '');
  }
  _explTTSText = txt;
  console.log('[CK Buddy] Read explanation — source length:', txt.length, 'from:', parentQ && parentQ.source);
  _explSpeak();
});
...
document.getElementById('btnUworldQbank').addEventListener('click', async () => {
  ...
  chrome.tabs.sendMessage(tab.id, { type: 'UWORLD_QBANK_SCRAPE' }, (response) => {
    if (chrome.runtime.lastError || !response?.ok) {
      showScreen('screen-home');
...
document.getElementById('btnUworldScan1').addEventListener('click', async () => {
  ...
  chrome.tabs.sendMessage(tab.id, { type: 'UWORLD_QBANK_SCRAPE', count: 1 }, (response) => {
    if (chrome.runtime.lastError || !response?.ok) {
      showScreen('screen-home');
...
document.getElementById('btnUworldScan5').addEventListener('click', async () => {
  ...
  chrome.tabs.sendMessage(tab.id, { type: 'UWORLD_QBANK_SCRAPE', count: 5 }, (response) => {
    if (chrome.runtime.lastError || !response?.ok) {
      showScreen('screen-home');
      document.getElementById('uworld-scan-status').textContent = '⚠ Could not scrape. Check UWorld tab.';
      return;
