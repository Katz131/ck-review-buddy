...
document.getElementById('btnReadExplanation').addEventListener('click', function() {
  ...
  // STRICT: only ever read the NATIVE qbank-scraped explanation. Never fall back
  // to AI-generated trivia.explanation, likelyMisconception, or synthesized text —
  // that was causing CK Buddy to read its own generated content instead of the
  // real AMBOSS / UWorld / NBME / CCS / NBME-FL explanation.
  ...
  if (nativeExpl.length > 5) {
    ...
    console.log('[CK Buddy] Read explanation — NATIVE source, length:', nativeExpl.length, 'from:', parentQ && parentQ.source);
    _explSpeak();
  } else {
    _explTTSText = '';
    try { window.speechSynthesis.cancel(); } catch(e) {}
    console.warn('[CK Buddy] Read explanation — native explanation missing/empty for source:', parentQ && parentQ.source, '— refusing to read AI-generated fallback.');
